-- ====================================================================
-- SCRIPT: SEGURANÇA MÁXIMA CONTRA MANIPULAÇÃO DE PEDIDOS (ORDER TAMPERING)
-- ====================================================================
-- Este script resolve a vulnerabilidade crítica de segurança que permitia
-- a qualquer utilizador anónimo atualizar dados de pedidos ativos de terceiros.
-- 
-- Para isso, implementamos uma Trigger no banco de dados que:
-- 1. Permite modificações totais se o utilizador for o dono do restaurante ou administrador global.
-- 2. Para requisições públicas (anónimas, como o motoboy ou checkout):
--    - Bloqueia terminantemente qualquer tentativa de alterar valores sensíveis 
--      (preços, itens, moradas, dados do cliente, descontos, cupões).
--    - Limita estritamente as alterações de estado do pedido apenas para as 
--      transições logísticas permitidas:
--        * 'waiting_payment' -> 'cancelled' (Falha de pagamento na compra)
--        * 'out_for_delivery' -> 'arrived' (Motoboy chegou à porta do cliente)
--        * 'arrived' -> 'delivered' (Entrega concluída com sucesso)
...
-- Ativar o trigger de segurança na tabela orders
