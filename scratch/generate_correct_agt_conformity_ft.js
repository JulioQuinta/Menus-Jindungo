...
function generateCorrectAgtConformityFT() {
    ...
    // 3. Payload de Assinatura do Documento para o tipo Factura (FT)
    ...
    // 4. Linha do produto ajustada conforme a especificação aprovada
    const lines = [
        {
            lineNumber: "1",
            productCode: "P001",
    ...
    // 5. Documento Oficial (Factura - FT)
    const document = {
        documentNo: "FT 2026/1",
        documentStatus: "N",
        documentDate: "2026-07-17",
    ...
    fs.writeFileSync(
        path.join(outputDir, 'agt_factura_envelope_oficial.json'),
        JSON.stringify(agtEnvelope, null, 2)
    );

    console.log("Envelope oficial para Factura (FT) gerado com sucesso!");
    console.log(JSON.stringify(agtEnvelope, null, 2));
}

generateCorrectAgtConformityFT();
